...
if ($ms > 0) { // nếu tìm theo id
  $sql_count = "SELECT COUNT(id_post) FROM posts WHERE status = 0 AND type_account = 'CF' AND id_post = $ms $sql_id";
} else {

  $sql_count = "SELECT COUNT(id_post) FROM posts WHERE status = 0 AND type_account = 'CF' AND  price BETWEEN $min_price AND $max_price $sql_id  ";
}
/* END */
...
$paging->init($config);
if ($ms > 0) { // nếu tìm theo id
  $sql_get_list_acc = "SELECT * FROM posts WHERE status = 0 AND type_account = 'CF' AND  id_post = $ms $sql_id $html_order LIMIT {$paging->getConfig()["start"]}, {$paging->getConfig()["limit"]}";
} else {

  $sql_get_list_acc = "SELECT * FROM posts WHERE status = 0 AND type_account = 'CF' AND  price BETWEEN $min_price AND $max_price $sql_id  $html_order LIMIT {$paging->getConfig()["start"]}, {$paging->getConfig()["limit"]}";
}
/* END */

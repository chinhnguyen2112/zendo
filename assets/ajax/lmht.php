...
if($source_signup === 0 || $source_signup > 0 ){ // nếu chọn nguồn đăng ký 
...
if ($ms > 0) { // nếu tìm theo id
  $sql_count = "SELECT COUNT(id_post) FROM posts WHERE status = 0 AND type_account = 'Liên Minh Huyền Thoại' AND id_post = $ms $sql_id";
  $sql_get_list_acc = "SELECT * FROM posts WHERE status = 0 AND type_account = 'Liên Minh Huyền Thoại' AND  id_post = $ms $sql_id $html_order LIMIT {$paging->getConfig()["start"]}, {$paging->getConfig()["limit"]}";
} else {

  $sql_count = "SELECT COUNT(id_post) FROM posts WHERE status = 0 AND type_account = 'Liên Minh Huyền Thoại' AND  price BETWEEN $min_price AND $max_price $sql_id $html_filter ";
  $sql_get_list_acc = "SELECT * FROM posts WHERE status = 0 AND type_account = 'Liên Minh Huyền Thoại' AND  price BETWEEN $min_price AND $max_price $sql_id $html_filter $html_order LIMIT {$paging->getConfig()["start"]}, {$paging->getConfig()["limit"]}";
}
/* END */
...
?>
    ...
    <div class="box__list_danhmuc"><a href="/tai-khoan-<?php echo $data_post['id_post']; ?>/">
        ...
        <div class="box_detail_danhmuc">
          ...
          <ul>
            ...
            <li class="li_ac">Rank: <?= str_replace('Khung ', '', $get_info->get_string_frame($data_post['rank'])) ?></li>
          </ul>
        </div>

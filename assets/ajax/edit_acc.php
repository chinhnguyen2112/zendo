    ...
    if (empty($username)) {
        ...
        for ($i = 0; $i < $total_champs; $i++) {
            if ($_FILES["champs"]["error"][$i] == 0) {
                ...
                $sql_get_data = "SELECT * FROM thumb_acc WHERE img = '$url_img'  LIMIT 1";
        ...
        if ($_FILES["thumb"]["error"] == 0) {
            ...
            resize_image_upload($_FILES["thumb"]["tmp_name"], $id_new . "." . end($arr),$id_new . ".jpg", 375, 375, 100, $type = "", $path . "thumb/");
            // move_uploaded_file($_FILES["thumb"]["tmp_name"], $path . "thumb/" . $id_new . "." . end($arr));
        }

        echo json_encode(array('status' => "success", 'title' => "Thành công", 'msg' => "Sửa tài khoản #$id_new thành công"));
        $db->close(); // Giải phóng

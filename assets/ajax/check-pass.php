...
// Passing `true` enables exceptions
    ...
    switch($_GET['action']){
    	       ...
    	       if(isset($_POST['id']) && is_numeric($_POST['id']))
                {
                    ...
                    $query = $db->fetch_assoc("SELECT * FROM `history_buy` WHERE `id_post` = '{$id}'",1);
                    
                    if($query['username'] == $iduser){

                      $info = $db->fetch_assoc("SELECT * FROM posts WHERE id_post = '".$id."' LIMIT 1", 1); 

        
                    $RSApassword = $_POST['rsapass'];
                    $username = preg_replace('/\s+/','',$info['username']);

    					$url="https://borrow.garena.com/index/";
                    $resulta = curl($url);
                    $csrf = get_input_val($resulta[1],'input[name=csrfmiddlewaretoken]');
    
                    $postdata="csrfmiddlewaretoken=".$csrf."&next=/home&username=".$username."&password=".$RSApassword."&password2=garena_gcms_pass";
                    ...
                    // echo json_encode(array('err'=>$pesan,'id'=>$id));
        ...
        case "update_acc":
                ...
                if (!empty($id)){
                $khadung = $db->num_rows("SELECT * FROM `history_buy` WHERE `id_post` = '{$id}'  LIMIT 1");
    	            if($khadung >= 1 && $_POST['error'] == 1){
    	               
    	                
    	                $db->query("UPDATE `history_buy` SET `status` ='1' WHERE `id_post` = '$id'" );
    	                ...
    	                $db->query("UPDATE `history_buy` SET `status` ='2' WHERE `id_post` = '$id'" );
